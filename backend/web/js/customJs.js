...
function mealPrep(mealId){

    document.getElementById("btnPrep").style.visibility = "hidden";
    var mealState = "preparing"
    $.ajax({
        type: "POST",
        url: 'http://localhost/healthify/backend/web/mealprep/preping/' + mealState + '&' + mealId,
        data: $('#' + 'grid').serialize(),
        cache: false,
        dataType: 'html',
        success: function (result) {
